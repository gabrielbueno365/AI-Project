...
"""
Funções auxiliares para o servidor MCP de mídia.
Adaptado do sistema existente para arquitetura MCP.
"""
...
async def convert_audio(
    ...
    try:
        ...
        logger.debug(f"FFmpeg: convertendo {input_path} para {output_path}")
...
async def sanitize_audio_file(
    ...
    if is_valid and force_convert:
        logger.info(f"Formato {path.suffix} válido, mas forçando conversão para MP3")
